...
This release also folds in PR #506:

- **`gpt-5.5` is the default live/quota probe model** (with the legacy Codex
  fallback chain — `gpt-5.4` → `gpt-5.3-codex` → `gpt-5.2-codex` → `gpt-5-codex`
  — preserved). The candidate chain now lives in one shared
  `QUOTA_PROBE_MODEL_CHAIN` constant so the manager and runtime probes cannot
  drift apart.
- **Codex-unavailable accounts are reported as "signed in" rather than
  "working."** Live check gained `Codex available` / `signed in only` /
  `need re-login` counters; an account that authenticates but can't reach Codex
  renders `!` instead of `✓`.
- **The Codex plugin manifest (`.codex-plugin/plugin.json`) ships in the
  package**, enforced by the pack-budget check via an exact-file requirement (a
  `.bak` sibling can no longer satisfy the gate).
- **`--model` argument parsing is hardened** across every CLI parser: a
  flag-like or whitespace-only value after `--model`/`-m`/`--model=` is rejected
  with a clear "Missing value" error instead of silently consuming the next flag.
- A model-less `/codex/responses` request now buckets into the codex rotation
  family (via `CURRENT_CODEX_MODEL`) instead of the general `gpt-5.5` family,
  keeping rotation/cooldown/budget accounting correct.
