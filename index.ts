...
export const OpenAIOAuthPlugin: Plugin = async ({ client }: PluginInput) => {
	...
	const sanitizeResponseHeadersForLog = (
		headers: Headers,
	...
	const persistAccountPool = async (
		...
		await withAccountStorageTransaction(async (loadedStorage, persist) => {
			...
			for (const result of results) {
				...
				const existingIndex =
					existingById ?? existingByEmail ?? existingByToken;
								...
								while (true) {
									...
									while (attempted.size < Math.max(1, accountCount)) {
										...
										while (true) {
												...
												responseForSuccess = withStreamingFailover(
													...
													async (failoverAttempt, emittedBytes) => {
														...
														for (const candidateIndex of streamFallbackCandidateOrder) {
															...
															const fallbackTokenAccountId = extractAccountId(
																fallbackAuth.access,
															);
											...
											const successAccountKey = resolveEntitlementAccountKey(
												successAccountForResponse,
											);
			...
			methods: [
				{
					...
					authorize: async (inputs?: Record<string, string>) => {
						...
						if (!explicitLoginMode) {
							while (true) {
								const loadedStorage = await hydrateEmails(await loadAccounts());
								let workingStorage = loadedStorage
									? {
											...loadedStorage,
											accounts: loadedStorage.accounts.map((account) => ({
												...account,
								...
								let flaggedStorage = await loadFlaggedAccounts();
									...
									if (typeof menuResult.deleteAccountIndex === "number") {
										...
										if (deleted) {
											workingStorage = {
												...deleted.storage,
												activeIndexByFamily:
													deleted.storage.activeIndexByFamily ?? {},
											};
											flaggedStorage = deleted.flagged;
											invalidateAccountManagerCache();
									...
									(account) =>
										(accountId &&
											(account.accountIdOverride ??
												extractAccountId(account.access)) === accountId) ||
										(email &&
