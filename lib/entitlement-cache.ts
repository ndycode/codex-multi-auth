...
/**
 * Normalize a model identifier into a canonical short form.
 *
 * @param model - The model identifier (may be undefined, include path segments, or include trailing variant suffixes)
 * @returns The normalized model name with any path prefix removed and trailing suffixes `-none`, `-minimal`, `-low`, `-medium`, `-high`, or `-xhigh` stripped; `null` if `model` is undefined or empty after trimming
 */
...
/**
 * Produces a stable string key for an entitlement account reference.
 *
 * The returned key uses the first available identifier in priority order: accountId, email, then index.
 *
 * @param ref - Account reference containing optional `accountId`, `email`, or `index`
 * @returns A string key prefixed with `id:`, `email:`, or `idx:` (e.g. `id:123`, `email:user@example.com`, `idx:0`)
 *
 * Concurrency: safe to call concurrently; the function is pure and has no side effects.
 * Filesystem: output is independent of platform filesystem semantics (including Windows path rules).
 * Sensitive data: keys may contain personally identifying values; redact or avoid logging them in production.
 */
