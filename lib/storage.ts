...
export function getNamedBackupsDirectoryPath(
	storagePath: string = getStoragePath(),
): string {
	return getNamedBackupRoot(storagePath);
}
...
function matchesSnapshotRetentionTargetPath(
...
async function getLatestManualCodexCliRollbackSnapshotNames(
	...
	try {
		...
		for (let index = history.length - 1; index >= 0; index -= 1) {
			...
			if (!matchesSnapshotRetentionTargetPath(run.targetPath, storagePath)) {
				continue;
			}
...
export async function pruneAutoGeneratedSnapshots(
	...
	const storagePath = options.storagePath ?? getStoragePath();
	const scanResult = options.backups ? null : await scanNamedBackups(storagePath);
	const backups =
		options.backups ??
	...
	const rollbackSnapshotNames =
		await getLatestManualCodexCliRollbackSnapshotNames(storagePath);
