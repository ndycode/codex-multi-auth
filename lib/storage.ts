...
const NAMED_BACKUP_DIRECTORY = "backups";
const NAMED_BACKUP_EXTENSION = ".json";
const AUTO_SNAPSHOT_NAME_PATTERN =
	/^accounts-(?<reason>[a-z0-9-]+)-snapshot-(?<timestamp>\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2})$/i;
...
function normalizeBackupName(rawName: string): string {
	...
	const sanitized = collapsedWhitespace
		...
		.slice(0, 120);
	if (!sanitized) {
		throw new Error(`Invalid backup name: ${rawName}`);
	}
	return sanitized;
}
...
async function ensureNamedBackupsDirectory(): Promise<string> {
	const directory = getNamedBackupsDirectory();
...
function deriveBackupNameFromFile(fileName: string): string {
	if (fileName.endsWith(NAMED_BACKUP_EXTENSION)) {
		return fileName.slice(0, -NAMED_BACKUP_EXTENSION.length);
	}
...
export async function createNamedBackup(
	...
	await ensureNamedBackupsDirectory();
	await exportAccounts(backupPath, options.force ?? false);
...
> {
	try {
		...
		for (let i = history.length - 1; i >= 0; i -= 1) {
			...
			if (entry.run?.trigger !== "manual") continue;
			const snapshotName = entry.run.rollbackSnapshot?.name;
		...
		log.debug("Failed to load sync history for rollback snapshot retention", {
			error: String(error),
		});
	}
	return new Set();
...
function parseAutoSnapshot(
	details: NamedBackupMetadata,
): AutoSnapshotDetails | null {
	const match = details.name.match(AUTO_SNAPSHOT_NAME_PATTERN);
	if (!match?.groups) return null;

	const { reason, timestamp } = match.groups as {
		reason?: string;
		timestamp?: string;
	};
	if (!reason || !timestamp) return null;

	const [datePart, timePart] = timestamp.split("_");
	if (!datePart || !timePart) return null;
	const formatted = `${datePart}T${timePart.replace(/-/g, ":")}Z`;
...
export async function pruneAutoGeneratedSnapshots(
	...
	const backups = options.backups ?? (await listNamedBackups());
	const keepLatestPerReason = Math.max(1, options.keepLatestPerReason ?? 1);
	...
	const autoSnapshots: AutoSnapshotDetails[] = [];
	for (const backup of backups) {
		const parsed = parseAutoSnapshot(backup);
		if (parsed) {
			autoSnapshots.push(parsed);
		}
	}

	const latestByReason = new Map<string, AutoSnapshotDetails>();
	for (const snapshot of autoSnapshots) {
		const existing = latestByReason.get(snapshot.reason);
		if (!existing || snapshot.timestampMs > existing.timestampMs) {
			latestByReason.set(snapshot.reason, snapshot);
		}
	}

	const keepSet = new Set<string>(preserveNames);
	for (const snapshot of latestByReason.values()) {
		keepSet.add(snapshot.name);
	}

	const sortedByTimestamp = [...autoSnapshots].sort(
		(a, b) => b.timestampMs - a.timestampMs,
	);
	for (const snapshot of sortedByTimestamp.slice(0, keepLatestPerReason)) {
		keepSet.add(snapshot.name);
	}
...
async function assessBackupRestoreFromPath(
	backupPath: string,
	options: {
		backup: NamedBackupMetadata;
		candidate: Awaited<ReturnType<typeof loadBackupCandidate>>;
		currentStorage?: AccountStorageV3 | null;
	},
): Promise<BackupRestoreAssessment> {
	const { backup, candidate } = options;
...
export async function assessNamedBackupRestore(
	...
	return assessBackupRestoreFromPath(backupPath, {
		backup,
		candidate,
...
export async function assessOpencodeAccountPool(
	...
	return assessBackupRestoreFromPath(resolvedPath, {
		backup,
		candidate,
		currentStorage: options.currentStorage,
	});
