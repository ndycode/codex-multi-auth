...
const retentionEnforcementInFlight = new Map<
	string,
...
export function isAccountSnapshotName(name: string): boolean {
	return /^accounts-(?<reason>[a-z0-9-]+)-snapshot-\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}_\d{3}$/i.test(
		name,
	);
}
...
export async function pruneAutoGeneratedSnapshots(
	...
	const inFlight = retentionEnforcementInFlight.get(resolvedStoragePath);
	if (inFlight) {
		return await inFlight;
	}

	const prunePromise = (async (): Promise<AutoSnapshotPruneResult> => {
		const backups =
			options.backups ??
			(
				await scanNamedBackups(
					getNamedBackupRoot(resolvedStoragePath),
				)
	...
	retentionEnforcementInFlight.set(resolvedStoragePath, prunePromise);
	try {
		return await prunePromise;
	} finally {
		if (retentionEnforcementInFlight.get(resolvedStoragePath) === prunePromise) {
