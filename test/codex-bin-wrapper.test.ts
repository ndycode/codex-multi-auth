...
describe("codex bin wrapper", () => {
	...
	it.skipIf(process.platform !== "win32")(
		"retries transient shim write failures and still installs guards",
		() => {
			for (const retryCode of ["EBUSY", "EPERM", "EACCES"]) {
				const fixtureRoot = createWrapperFixture();
				const fakeBin = createFakeCodexBin(fixtureRoot);
				const shimDir = join(fixtureRoot, "shim-bin");
				mkdirSync(shimDir, { recursive: true });
				...
				const hookPath = join(fixtureRoot, `fail-once-shim-write-${retryCode}.cjs`);
				writeFileSync(
					hookPath,
					[
						...
						"  if (!injected && /codex\\.(bat|cmd|ps1)$/i.test(target)) {",
						"    injected = true;",
						'    const error = new Error("busy");',
						`    error.code = "${retryCode}";`,
						"    throw error;",
						"  }",
				...
				const preloadOption = `--require=${hookPath}`;
				const nodeOptions =
	...
	it.skipIf(process.platform !== "win32")(
		"keeps PowerShell profile guard idempotent on repeated startup",
		() => {
			const fixtureRoot = createWrapperFixture();
			const fakeBin = createFakeCodexBin(fixtureRoot);
			const shimDir = join(fixtureRoot, "shim-bin");
			...
			expect(runWrapper(fixtureRoot, ["--version"], env).status).toBe(0);
			const pwshProfilePath = join(
				fixtureRoot,
				"Documents",
				"PowerShell",
			...
			const firstContent = readFileSync(pwshProfilePath, "utf8");

			expect(runWrapper(fixtureRoot, ["--version"], env).status).toBe(0);
			const secondContent = readFileSync(pwshProfilePath, "utf8");

			expect(secondContent).toBe(firstContent);
			expect(secondContent.startsWith("\r\n")).toBe(false);
			expect(
				(secondContent.match(/# >>> codex-multi-auth shell guard >>>/g) ?? []).length,
			).toBe(1);
		},
	);
	...
	it("treats numeric-string zero multi-auth exit code as success", () => {
		const fixtureRoot = createWrapperFixture();
		const distLibDir = join(fixtureRoot, "dist", "lib");
		mkdirSync(distLibDir, { recursive: true });
		writeFileSync(
