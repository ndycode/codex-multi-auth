...
describe("codex manager cli commands", () => {
	beforeEach(() => {
		...
		importAccountsMock.mockResolvedValue({ imported: 0, skipped: 0, total: 0 });
	...
	it("offers startup backup browser before OAuth when interactive login starts empty", async () => {
		...
		const assessment = {
			...
			valid: true,
			nextActiveIndex: 0,
			nextActiveEmail: "new@example.com",
			nextActiveAccountId: undefined,
			activeAccountChanged: true,
			error: "",
		};
	...
	it("previews and applies restore from backup browser", async () => {
		setInteractiveTTY(true);
		const now = Date.now();
		createAuthorizationFlowMock.mockResolvedValue({
			url: "https://example.com/oauth",
		...
		const assessment = {
			...
			valid: true,
			nextActiveIndex: 0,
			nextActiveEmail: "restored@example.com",
			nextActiveAccountId: "acc_restored",
			currentActiveIndex: 0,
			currentActiveEmail: undefined,
			currentActiveAccountId: undefined,
		...
		loadAccountsMock.mockResolvedValue(structuredClone(restoredStorage));
		getActionableNamedBackupRestoresMock.mockResolvedValue({
			assessments: [assessment],
			totalBackups: 1,
		});
		...
		confirmMock.mockResolvedValueOnce(true);
		selectMock
			.mockResolvedValueOnce({ type: "inspect", entry })
	...
	it("refuses restore preview when backup is invalid", async () => {
		setInteractiveTTY(true);
		const now = Date.now();
		createAuthorizationFlowMock.mockResolvedValue({
			url: "https://example.com/oauth",
		...
		const assessment = {
			...
			backupAccountCount: 22,
			dedupedBackupAccountCount: 22,
			conflictsWithinBackup: 0,
			conflictsWithExisting: 0,
			replacedExistingCount: 0,
			keptExistingCount: 1,
			...
			nextActiveAccountId: undefined,
			currentActiveIndex: 0,
			currentActiveEmail: "existing@example.com",
			currentActiveAccountId: "acc_existing",
			activeAccountOutcome: "unchanged",
			activeAccountChanged: false,
			error: "Restore would exceed maximum",
		};
		const entry = {
			kind: "named" as const,
			label: assessment.backup.name,
	...
	it("shows sync-center source/runtime context and keeps apply one-way", async () => {
		...
		previewCodexCliSyncMock
			.mockResolvedValueOnce({
				status: "ready",
				statusDetail:
					"Preview ready: 1 add, 0 update, 1 destination-only preserved.",
				sourcePath: "/mock/codex/accounts.json",
				targetPath: "/mock/openai-codex-accounts.json",
				summary: {
		...
		syncAccountStorageFromCodexCliMock.mockResolvedValueOnce({
			changed: true,
			storage: {
				...storage,
				accounts: [
		...
		selectMock.mockImplementation(async (items) => {
			...
			if (selectCall === 2) {
				const text = items
					...
					.join("\n");
				expect(text).toContain("Target path: /mock/openai-codex-accounts.json");
				expect(text).toContain("/mock/codex/accounts.json");
				expect(text).toContain("/mock/codex/auth.json");
				expect(text).toContain("/mock/codex/config.toml");
				expect(text).toContain("Preview mode: read-only until apply");
				expect(text).toContain(
					"Selection precedence: accountId -> email -> preserve newer local choice",
				);
				expect(text).toContain("One-way sync never deletes accounts");
				expect(text).toContain("Live watcher: running");
				return { type: "apply" };
			}
			if (selectCall === 3) return { type: "back" };
			return { type: "back" };
		});
		...
		expect(exitCode).toBe(0);
		expect(syncAccountStorageFromCodexCliMock).toHaveBeenCalledTimes(1);
		expect(saveAccountsMock).toHaveBeenCalledTimes(1);
