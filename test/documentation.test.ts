...
const opencodeReferenceAllowedFiles = new Set([
	"docs/reference/storage-paths.md",
]);
...
describe("Documentation Integrity", () => {
	...
	it("keeps settings reference sections aligned with current menu labels and backend categories", () => {
		...
		expect(settingsRef).toContain(
			"allows only ASCII letters, digits, underscores, and hyphens in the backup basename",
		);
		expect(settingsRef).toContain(
	...
	it("documents Experimental sync, backup, and target-path semantics consistently", () => {
		...
		expect(readme).toContain(
			"Blocked sync targets only show guidance, they don't write anything.",
		);
		expect(readme).toContain(
			"Successful sync keeps the destination active selection",
		);
		expect(readme).toContain("destination-only accounts are preserved");
		expect(readme).toContain("append `.json` when needed");
		expect(readme).toContain(
			"allow only ASCII letters, digits, underscores, and hyphens in the backup basename",
		);
		expect(readme).toContain(
			"reject spaces, extra dots, separators, traversal, `.rotate.`, `.tmp`, and `.wal` patterns",
		);
		...
		expect(storagePaths).toContain("%USERPROFILE%\\.opencode");
		expect(storagePaths).toContain(
			"%USERPROFILE%\\.opencode\\projects\\<project-key>",
		);
		expect(storagePaths).toContain(
			"sync blocks and only shows guidance instead of applying changes",
		);
		expect(storagePaths).toContain(
			"keeps the destination active selection and preserves destination-only accounts",
		);
	});
