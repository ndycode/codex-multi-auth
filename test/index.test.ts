...
function getFetchAccessTokenSequence(): Array<string | null> {
	const fetchMock = globalThis.fetch as unknown as {
		mock: { calls: Array<[unknown, RequestInit | undefined]> };
	};
	return fetchMock.mock.calls.map(([, init]) =>
		new Headers(init?.headers).get("x-test-access-token"),
	);
}
...
describe("OpenAIOAuthPlugin fetch handler", () => {
	...
	it("queues manual account selection behind an in-flight CLI sync", async () => {
		mockStorage.accounts = [
	...
	it("does not let an in-flight request restore an older CLI selection after a tool switch", async () => {
		mockStorage.accounts = [
			{ accountId: "acc-1", email: "user1@example.com", refreshToken: "refresh-1" },
			{ accountId: "acc-2", email: "user2@example.com", refreshToken: "refresh-2" },
		];
		...
		await vi.waitFor(() => expect(syncCodexCliSelectionMock).toHaveBeenCalledTimes(1));
		const switchResult = await plugin.tool["codex-switch"].execute({ index: 2 });

		expect(switchResult).toContain("Switched to account");
		expect(syncCodexCliSelectionMock.mock.calls.map((call) => call[0])).toEqual([0, 1]);
	...
	it("times out a stalled CLI sync so later requests can continue", async () => {
		...
		try {
			...
			expect(vi.mocked(loggerModule.logWarn)).toHaveBeenCalledWith(
				expect.stringContaining("Codex CLI selection sync timed out"),
	...
	it("ignores a timed-out stale sync completion after a newer account sync wins", async () => {
		vi.useFakeTimers();
		try {
			const { AccountManager } = await import("../lib/accounts.js");
			const accountOne = {
			...
			let resolveStaleSync: (() => void) | null = null;
			const currentAccount = vi
				.fn()
				.mockReturnValueOnce(accountOne)
				.mockReturnValueOnce(accountTwo)
				.mockReturnValueOnce(accountTwo)
			...
			const { sdk } = await setupPlugin();
			const firstResponsePromise = sdk.fetch!("https://api.openai.com/v1/chat", {
				method: "POST",
				body: JSON.stringify({ model: "gpt-5.1" }),
			});

			await vi.advanceTimersByTimeAsync(5_000);
			const secondResponsePromise = sdk.fetch!("https://api.openai.com/v1/chat", {
				method: "POST",
				body: JSON.stringify({ model: "gpt-5.1" }),
			});
			...
			expect(getFetchAccessTokenSequence()).toEqual([
				"access-account-1",
				"access-account-2",
				"access-account-2",
			]);
	...
	it("retries CLI injection on the next request after a forced refresh write fails", async () => {
		const fetchHelpers = await import("../lib/request/fetch-helpers.js");
		vi.mocked(fetchHelpers.shouldRefreshToken)
			.mockReturnValueOnce(false)
			.mockReturnValueOnce(true)
	...
	it("re-resolves the live CLI sync index when the selected account is reindexed", async () => {
		...
		const movedSelectedAccount = {
			...selectedAccount,
			index: 1,
		};
		const customManager = buildCustomManager(selectedAccount, displacedAccount, {
			getCurrentOrNextForFamilyHybrid: () => selectedAccount,
			getAccountsSnapshot: () => [displacedAccount, movedSelectedAccount],
			getAccountByIndex: (index: number) =>
				index === 0 ? displacedAccount : index === 1 ? movedSelectedAccount : null,
		});
		const { AccountManager } = await import("../lib/accounts.js");
		vi.spyOn(AccountManager, "loadFromDisk").mockResolvedValue(customManager as never);
		globalThis.fetch = vi.fn().mockResolvedValue(
	...
	it("forces failover CLI reinjection even when the fallback account was previously injected", async () => {
		vi.doMock("../lib/request/stream-failover.js", () => ({
			withStreamingFailover: async (
				response: Response,
				failover: (attempt: number, emittedBytes: number) => Promise<Response | null>,
		...
		try {
			...
			const { sdk } = await setupPlugin();
			const seedResponse = await sdk.fetch!("https://api.openai.com/v1/chat", {
				method: "POST",
				body: JSON.stringify({ model: "gpt-5.1" }),
			});
			const response = await sdk.fetch!("https://api.openai.com/v1/chat", {
			...
			expect(response.status).toBe(200);
			expect(syncCodexCliSelectionMock.mock.calls.map((call) => call[0])).toEqual([1, 0, 1]);
			expect(getFetchAccessTokenSequence()).toEqual([
				"access-account-2",
				"access-account-1",
				"access-account-1",
	...
	it("forces failover CLI reinjection when refreshed fallback auth stays on the same account", async () => {
		vi.doMock("../lib/request/stream-failover.js", () => ({
			withStreamingFailover: async (
				response: Response,
				failover: (attempt: number, emittedBytes: number) => Promise<Response | null>,
		...
		try {
			...
			const customManager = buildCustomManager(accountOne, accountOne, {
				...
				toAuthDetails: () => ({
					type: "oauth" as const,
					access: accountOne.access,
					refresh: accountOne.refreshToken,
	...
	it("skips manual account selection injection when direct injection is disabled", async () => {
		mockStorage.accounts = [
			{ accountId: "acc-1", email: "user1@example.com", refreshToken: "refresh-1" },
			{ accountId: "acc-2", email: "user2@example.com", refreshToken: "refresh-2" },
		];
		...
		try {
			...
			const customManager = buildCustomManager(accountOne, accountTwo, {
				...
				globalThis.fetch = vi
					...
					.mockResolvedValueOnce(new Response("retry later", { status: 429 }))
					.mockResolvedValueOnce(
						new Response(JSON.stringify({ content: `${code}-ok` }), { status: 200 }),
					);
				...
				expect(response.status).toBe(200);
				expect(await response.text()).toContain(`${code}-ok`);
				expect(vi.mocked(loggerModule.logWarn)).toHaveBeenCalledWith(
					expect.stringContaining("Codex CLI selection sync failed"),
					expect.objectContaining({
						accountIndex: 1,
						code,
					}),
	...
	it("retries a warm cached fallback selection after a forced reinjection write fails", async () => {
		vi.doMock("../lib/request/stream-failover.js", () => ({
			withStreamingFailover: async (
				response: Response,
				failover: (attempt: number, emittedBytes: number) => Promise<Response | null>,
		...
		try {
			...
			syncCodexCliSelectionMock
				...
				.mockRejectedValueOnce(
					Object.assign(new Error("EACCES fail"), { code: "EACCES" }),
				)
				.mockResolvedValueOnce(true);
			globalThis.fetch = vi
				.fn()
				...
				.mockResolvedValueOnce(
					new Response(JSON.stringify({ content: "steady-ok" }), { status: 200 }),
				);
			...
			const failoverResponse = await sdk.fetch!("https://api.openai.com/v1/chat", {
				method: "POST",
				body: JSON.stringify({ model: "gpt-5.1", stream: true }),
			});
			const steadyStateResponse = await sdk.fetch!("https://api.openai.com/v1/chat", {
				method: "POST",
				body: JSON.stringify({ model: "gpt-5.1" }),
			});
			...
			expect(steadyStateResponse.status).toBe(200);
			expect(syncCodexCliSelectionMock.mock.calls.map((call) => call[0])).toEqual([
				1,
				0,
				1,
	...
	it("skips refresh-driven CLI injection when direct injection is disabled", async () => {
		const configModule = await import("../lib/config.js");
		const fetchHelpers = await import("../lib/request/fetch-helpers.js");
		vi.mocked(configModule.getCodexCliDirectInjection).mockReturnValue(false);
		vi.mocked(fetchHelpers.shouldRefreshToken).mockReturnValueOnce(true);
		vi.mocked(fetchHelpers.refreshAndUpdateToken).mockResolvedValueOnce({
			type: "oauth",
			access: "refreshed-disabled-access",
			refresh: "refresh-token",
			expires: Date.now() + 60_000,
			multiAccount: true,
		});
	...
	it("forces Spark fallback even when strict policy disables generic unsupported fallback", async () => {
		...
		it("restarts account traversal after fallback model switch", async () => {
			...
			const customManager = buildCustomManager(accountOne, accountTwo, {
				...
				consumeToken: () => true,
				refundToken: () => {},
				syncCodexCliActiveSelectionForIndex: async () => true,
				markSwitched: () => {},
				removeAccount: () => {},
				recordFailure: () => {},
				recordSuccess: () => {},
...
describe("OpenAIOAuthPlugin event handler edge cases", () => {
	...
	it("reloads account manager from disk when handling account.select", async () => {
		...
		expect(loadFromDiskSpy).toHaveBeenCalledTimes(2);
		expect(syncCodexCliSelectionMock).toHaveBeenCalledWith(1);
		expect(loadFromDiskSpy.mock.invocationCallOrder[1] ?? Number.POSITIVE_INFINITY).toBeLessThan(
			syncCodexCliSelectionMock.mock.invocationCallOrder[0] ?? Number.POSITIVE_INFINITY,
		);
	});

	it("uses the latest reloaded manager for a queued account.select sync", async () => {
		mockStorage.accounts = [
			{ accountId: "acc-1", email: "user1@example.com", refreshToken: "refresh-1" },
			{ accountId: "acc-2", email: "user2@example.com", refreshToken: "refresh-2" },
		];
		...
		const loadFromDiskSpy = vi.spyOn(AccountManager, "loadFromDisk");
		loadFromDiskSpy.mockClear();
		const staleSync = vi.fn(async () => true);
		const freshSync = vi.fn(async () => true);
		loadFromDiskSpy
			.mockResolvedValueOnce({
				syncCodexCliActiveSelectionForIndex: staleSync,
			} as never)
			.mockResolvedValueOnce({
				syncCodexCliActiveSelectionForIndex: freshSync,
			} as never);
		...
		await vi.waitFor(() => expect(loadFromDiskSpy).toHaveBeenCalledTimes(1));
		resolveSync?.();
		...
		expect(freshSync).toHaveBeenCalledWith(1);
	});

	it("re-resolves the live account by unique identity after a same-timestamp reindex", async () => {
		const { AccountManager } = await import("../lib/accounts.js");
		const originalAccountAtIndexZero = {
			index: 0,
			accountId: "acc-1",
			email: "user1@example.com",
			refreshToken: "refresh-1",
		...
		const siblingAccountAtIndexOne = {
			index: 1,
			accountId: "acc-2",
			email: "user2@example.com",
			refreshToken: "refresh-2",
		...
		const siblingAccountAtIndexZero = {
			...siblingAccountAtIndexOne,
			index: 0,
		};
		const originalAccountAtIndexOne = {
			...originalAccountAtIndexZero,
			index: 1,
		};
		let currentAccounts = [originalAccountAtIndexZero, siblingAccountAtIndexOne];
		const customManager = buildCustomManager(
			originalAccountAtIndexZero,
			siblingAccountAtIndexOne,
			{
				getCurrentOrNextForFamilyHybrid: vi
					.fn()
					.mockReturnValueOnce(originalAccountAtIndexZero)
					.mockImplementation(() => null),
				getAccountsSnapshot: () => currentAccounts,
				getAccountByIndex: (index: number) => currentAccounts[index] ?? null,
			},
		);
		vi.spyOn(AccountManager, "loadFromDisk").mockResolvedValue(customManager as never);
		...
		await vi.waitFor(() => expect(syncCodexCliSelectionMock).toHaveBeenCalledTimes(1));
		currentAccounts = [siblingAccountAtIndexZero, originalAccountAtIndexOne];
		resolveFetch?.();
