...
describe("Storage Paths Module", () => {
  ...
  describe("getConfigDir", () => {
    ...
    it("prefers fallback with accounts when primary only has non-account signals", () => {
      ...
      mockedExistsSync.mockImplementation((candidate) => {
        ...
        if (candidate === path.join(primary, "openai-codex-accounts.json"))
          return false;
        if (candidate === path.join(primary, "codex-accounts.json"))
          return false;
        if (candidate === path.join(primary, "config.json")) return false;
        if (candidate === path.join(primary, "dashboard-settings.json"))
          return false;
    ...
    it("prefers Windows fallback with accounts when primary only has non-account signals", () => {
      ...
      const fallback = path.win32.join(
        "C:\\Users\\test",
        "DevTools",
        "config",
        "codex",
      ...
      const normalizePath = (input: string) =>
        path.win32.normalize(input.replace(/\//g, "\\"));
      ...
      mockedExistsSync.mockImplementation((candidate) => {
        ...
        if (
          normalizedCandidate ===
          normalizePath(path.win32.join(primary, "settings.json"))
        )
          return true;
        if (
          normalizedCandidate ===
          normalizePath(path.win32.join(primary, "openai-codex-accounts.json"))
        )
          return false;
        if (
          normalizedCandidate ===
          normalizePath(path.win32.join(primary, "codex-accounts.json"))
        )
          return false;
        if (
          normalizedCandidate ===
          normalizePath(path.win32.join(primary, "config.json"))
        )
          return false;
        if (
          normalizedCandidate ===
          normalizePath(path.win32.join(primary, "dashboard-settings.json"))
        )
          return false;
        if (
          normalizedCandidate ===
          normalizePath(path.win32.join(primary, "projects"))
        )
          return false;
        return (
          normalizedCandidate ===
          normalizePath(path.win32.join(fallback, "openai-codex-accounts.json"))
        );
      });
  ...
  describe("getProjectStorageKey", () => {
    ...
    it("falls back to a generic project name when basename sanitizes to empty", () => {
      const key = getProjectStorageKey("/tmp/!!!!");
      expect(key).toMatch(/^project-[a-f0-9]{12}$/);
    });
  });
  ...
  describe("getProjectGlobalConfigDir", () => {
    it("returns ~/.codex/multi-auth/projects/<key>", () => {
      ...
      expect(result).toContain(
        path.join(homedir(), ".codex", "multi-auth", "projects"),
      );
  ...
  describe("resolveProjectStorageIdentityRoot", () => {
    ...
    it("supports Windows-style backslash gitdir pointers", () => {
      ...
      const worktreeGitDir = path.win32.join(
        "C:\\repo",
        ".git",
        "worktrees",
        "pr-8",
    ...
    it("accepts aliased worktree roots when canonical paths match", () => {
      ...
      const gitEntry = path.win32.join(projectRoot, ".git");
      const canonicalProjectRoot = path.win32.join(
        "D:\\repo",
        "worktrees",
        "pr-8",
      );
  ...
  describe("resolvePath", () => {
    ...
    it("should throw for paths outside allowed directories", () => {
      ...
      if (
        !outsidePath.startsWith(home) &&
        !outsidePath.startsWith(cwd) &&
        !outsidePath.startsWith(tmp)
      ) {
    ...
    it("rejects lookalike prefix paths outside home directory", () => {
      ...
      const outsideLookalike = path.join(
        parent,
        `${path.basename(home)}-outside`,
        "file.json",
      );
  ...
  describe("resolveProjectStorageIdentityRoot edge branches", () => {
    it("returns project root when .git entry is missing", () => {
      const projectRoot = "/repo/no-git";
      mockedExistsSync.mockReturnValue(false);
    ...
    it("returns project root when .git stat is neither file nor directory", () => {
      const projectRoot = "/repo/odd-git";
      const gitEntry = path.join(projectRoot, ".git");
      mockedExistsSync.mockImplementation(
        (candidate) => candidate === gitEntry,
      );
    ...
    it.each([
      { label: "empty pointer", pointer: "\n" },
      { label: "invalid pointer prefix", pointer: "not-a-gitdir\n" },
      { label: "blank gitdir value", pointer: "gitdir:    \n" },
    ])("returns project root for $label", ({ pointer }) => {
      const projectRoot = "/repo/invalid-pointer";
      const gitEntry = path.join(projectRoot, ".git");
      mockedExistsSync.mockImplementation(
        (candidate) => candidate === gitEntry,
      );
    ...
    it("falls back when worktree gitdir back reference file is missing", () => {
      const projectRoot = "/repo/worktrees/no-backref";
      const gitEntry = path.join(projectRoot, ".git");
      const worktreeGitDir = "/repo/.git/worktrees/no-backref";
    ...
    it("falls back when worktree gitdir back reference file is empty", () => {
      const projectRoot = "/repo/worktrees/empty-backref";
      const gitEntry = path.join(projectRoot, ".git");
      const worktreeGitDir = "/repo/.git/worktrees/empty-backref";
      const gitdirBackRefFile = path.join(worktreeGitDir, "gitdir");

      mockedExistsSync.mockImplementation((candidate) => {
        return candidate === gitEntry || candidate === gitdirBackRefFile;
      });
      mockedStatSync.mockImplementation((candidate) => {
        expect(candidate).toBe(gitEntry);
        return buildMockStat({ isDirectory: false, isFile: true });
    ...
    it("falls back when worktree commondir file is missing", () => {
      const projectRoot = "/repo/worktrees/no-commondir";
      const gitEntry = path.join(projectRoot, ".git");
      const worktreeGitDir = "/repo/.git/worktrees/no-commondir";
      const gitdirBackRefFile = path.join(worktreeGitDir, "gitdir");
    ...
    it("falls back when worktree commondir file is empty", () => {
      const projectRoot = "/repo/worktrees/empty-commondir";
      const gitEntry = path.join(projectRoot, ".git");
      const worktreeGitDir = "/repo/.git/worktrees/empty-commondir";
      const gitdirBackRefFile = path.join(worktreeGitDir, "gitdir");
      const commondirFile = path.join(worktreeGitDir, "commondir");
    ...
    it("falls back when shared repository .git directory is missing", () => {
      const projectRoot = "/repo/worktrees/missing-shared-git";
      const gitEntry = path.join(projectRoot, ".git");
      const worktreeGitDir = "/repo/.git/worktrees/missing-shared-git";
      const gitdirBackRefFile = path.join(worktreeGitDir, "gitdir");
      const commondirFile = path.join(worktreeGitDir, "commondir");
      const sharedGitDir = "/repo/.git";
      const sharedRepoRootGitEntry = path.join("/repo", ".git");

      mockedExistsSync.mockImplementation((candidate) => {
        if (candidate === sharedRepoRootGitEntry) {
          return false;
        }
        return (
          candidate === gitEntry ||
    ...
    it("handles POSIX relative gitdir pointers when native realpath is unavailable", () => {
      const projectRoot = "/repo/worktrees/posix-relative";
      const gitEntry = path.join(projectRoot, ".git");
      const worktreeGitDir = "/repo/.git/worktrees/posix-relative";
      const gitdirBackRefFile = path.join(worktreeGitDir, "gitdir");
      const commondirFile = path.join(worktreeGitDir, "commondir");
      const sharedRepoRoot = "/repo";
      const sharedGitDir = path.join(sharedRepoRoot, ".git");
      ...
      mockedReadFileSync.mockImplementation((candidate) => {
        if (candidate === gitEntry) {
          return "gitdir: ../../.git/worktrees/posix-relative\n";
        }
        if (candidate === gitdirBackRefFile) {
          return `${path.join(projectRoot, ".git")}\n`;
        }
