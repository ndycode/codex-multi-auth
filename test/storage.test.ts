...
describe("storage", () => {
	...
	describe("account storage snapshots", () => {
		...
		it("keeps only the latest marked auto snapshots per reason while preserving manual snapshot-shaped backups", async () => {
			await saveAccounts({
				version: 3,
				activeIndex: 0,
				accounts: [
			...
			const manualSnapshotShapedBackup = await createNamedBackup(
				"accounts-delete-saved-accounts-snapshot-2023-12-31_23-59-59_999",
			);
			for (const millisecond of [1, 2, 3, 4]) {
				await snapshotAccountStorage({
					reason: "delete-saved-accounts",
					now: Date.UTC(2024, 0, 2, 3, 4, 5, millisecond),
				});
			}

			const backups = await listNamedBackups();
			const matchingNames = backups
				.filter((backup) =>
					backup.name.startsWith("accounts-delete-saved-accounts-snapshot-"),
				)
				.map((backup) => backup.name);

			expect(matchingNames).toContain(manualSnapshotShapedBackup.name);
			expect(matchingNames).toHaveLength(4);
			expect(matchingNames).not.toContain(
				"accounts-delete-saved-accounts-snapshot-2024-01-02_03-04-05_001",
			);
			expect(matchingNames).toContain(
				"accounts-delete-saved-accounts-snapshot-2024-01-02_03-04-05_002",
			);
			expect(matchingNames).toContain(
				"accounts-delete-saved-accounts-snapshot-2024-01-02_03-04-05_003",
			);
			expect(matchingNames).toContain(
				"accounts-delete-saved-accounts-snapshot-2024-01-02_03-04-05_004",
			);
		});

		it("pins snapshot retention to the captured storage root", async () => {
			const alternateStoragePath = join(
				testWorkDir,
				"alternate",
			...
			for (const millisecond of [1, 2, 3, 4]) {
				await snapshotAccountStorage({
					reason: "import-accounts",
					now: Date.UTC(2024, 0, 2, 3, 4, 5, millisecond),
					storagePath: alternateStoragePath,
					storage: alternateStorage,
				});
			}

			const alternateBackupRoot = join(dirname(alternateStoragePath), "backups");
			const alternateEntries = await fs.readdir(alternateBackupRoot);
			expect(
				alternateEntries.filter(
					(name) =>
						name.startsWith("accounts-import-accounts-snapshot-") &&
						name.endsWith(".json"),
				),
			...
			expect(
				existsSync(
					join(
						alternateBackupRoot,
						"accounts-import-accounts-snapshot-2024-01-02_03-04-05_001.json",
					),
				),
			).toBe(false);
		});

		it("honors preserveNames while pruning only marker-backed auto snapshots", async () => {
			await saveAccounts({
				version: 3,
				activeIndex: 0,
				accounts: [
			...
			const makeSnapshotLikeBackup = async (
				name: string,
				updatedAt: number,
				markedAuto: boolean,
			) => {
				const backup = await createNamedBackup(name);
				const timestamp = new Date(updatedAt);
				await fs.utimes(backup.path, timestamp, timestamp);
				if (markedAuto) {
					await fs.writeFile(`${backup.path}.snapshot-auto`, "import-accounts");
				}
				const listedBackup = (await listNamedBackups()).find(
					(candidate) => candidate.name === backup.name,
				);
				if (!listedBackup) {
					throw new Error(`expected backup metadata for ${backup.name}`);
				}
				return listedBackup;
			};

			const preserved = await makeSnapshotLikeBackup(
				"accounts-import-accounts-snapshot-2024-01-02_03-04-05_001",
				Date.UTC(2024, 0, 2, 3, 4, 5, 1),
				true,
			);
			const middle = await makeSnapshotLikeBackup(
				"accounts-import-accounts-snapshot-2024-01-02_03-04-05_002",
				Date.UTC(2024, 0, 2, 3, 4, 5, 2),
				true,
			);
			const newest = await makeSnapshotLikeBackup(
				"accounts-import-accounts-snapshot-2024-01-02_03-04-05_003",
				Date.UTC(2024, 0, 2, 3, 4, 5, 3),
				true,
			);
			const newestPlus = await makeSnapshotLikeBackup(
				"accounts-import-accounts-snapshot-2024-01-02_03-04-05_004",
				Date.UTC(2024, 0, 2, 3, 4, 5, 4),
				true,
			);
			const manual = await makeSnapshotLikeBackup(
				"accounts-import-accounts-snapshot-2024-01-02_03-04-05_999",
				Date.UTC(2024, 0, 2, 3, 4, 5, 999),
				false,
			);

			const result = await pruneAutoGeneratedSnapshots({
				backups: (await listNamedBackups()).filter((backup) =>
					backup.name.startsWith("accounts-import-accounts-snapshot-"),
				),
				keepLatestPerReason: 2,
				preserveNames: [preserved.name],
			});

			expect(result.pruned.map((backup) => backup.name)).toEqual([middle.name]);
			expect(result.kept.map((backup) => backup.name)).toEqual(
				expect.arrayContaining([preserved.name, newest.name, newestPlus.name]),
			);
			expect(existsSync(manual.path)).toBe(true);
		});

		it("keeps scanning rollback history when the newest manual entry lacks a snapshot name", async () => {
			await saveAccounts({
				version: 3,
				activeIndex: 0,
				accounts: [
			...
			const protectedBackup = await createNamedBackup(
				"accounts-manual-sync-snapshot-2024-01-02_03-04-05_001",
			);
			await fs.writeFile(`${protectedBackup.path}.snapshot-auto`, "manual-sync");
			const extraBackup = await createNamedBackup(
				"accounts-manual-sync-snapshot-2024-01-02_03-04-05_002",
			);
			await fs.writeFile(`${extraBackup.path}.snapshot-auto`, "manual-sync");
			const backups = (await listNamedBackups()).filter((backup) =>
				backup.name.startsWith("accounts-manual-sync-snapshot-"),
			);
			...
			vi.doMock("../lib/sync-history.js", async () => {
				...
				return {
					...actual,
					readSyncHistory: vi.fn().mockResolvedValue([
						{
							kind: "codex-cli-sync",
							recordedAt: 10,
							run: {
			...
			try {
				const isolatedStorageModule = await import("../lib/storage.js");
				const result = await isolatedStorageModule.pruneAutoGeneratedSnapshots({
					backups,
					keepLatestPerReason: 1,
				});

				expect(result.kept.map((backup) => backup.name)).toContain(
		...
		it("deduplicates concurrent prune requests for the same storage path", async () => {
			await saveAccounts({
				version: 3,
				activeIndex: 0,
				accounts: [
			...
			const oldBackup = await createNamedBackup(
				"accounts-import-race-snapshot-2024-01-02_03-04-05_001",
			);
			await fs.writeFile(`${oldBackup.path}.snapshot-auto`, "import-accounts");
			const newBackup = await createNamedBackup(
				"accounts-import-race-snapshot-2024-01-02_03-04-05_002",
			);
			await fs.writeFile(`${newBackup.path}.snapshot-auto`, "import-accounts");

			const originalUnlink = fs.unlink.bind(fs);
			const unlinkSpy = vi.spyOn(fs, "unlink");
			let releaseFirstUnlink!: () => void;
			let resolveFirstUnlinkStarted!: () => void;
			const firstUnlinkStarted = new Promise<void>((resolve) => {
				resolveFirstUnlinkStarted = resolve;
			});
			const firstUnlinkGate = new Promise<void>((resolve) => {
				releaseFirstUnlink = resolve;
			});
			let heldUnlink = false;
			unlinkSpy.mockImplementation(async (path, ...rest) => {
				if (!heldUnlink && path === oldBackup.path) {
					heldUnlink = true;
					resolveFirstUnlinkStarted();
			...
			const backups = (await listNamedBackups()).filter((backup) =>
				backup.name.startsWith("accounts-import-race-snapshot-"),
			);
			const firstPrune = pruneAutoGeneratedSnapshots({
				backups,
				keepLatestPerReason: 1,
			});
			await firstUnlinkStarted;
			const secondPrune = pruneAutoGeneratedSnapshots({
				backups,
				keepLatestPerReason: 1,
			});
			releaseFirstUnlink();
			...
			expect(
				unlinkSpy.mock.calls.filter(([path]) => path === oldBackup.path),
			).toHaveLength(1);
			expect(
				unlinkSpy.mock.calls.filter(
					([path]) => path === `${oldBackup.path}.snapshot-auto`,
				),
			).toHaveLength(1);
		});
				...
				.mockImplementation(async (path, data, options) => {
					if (
						String(path).includes(
							"accounts-delete-saved-accounts-snapshot-",
						) &&
						!String(path).endsWith(".snapshot-auto")
					) {
	...
	describe("opencode account pool detection", () => {
		...
		it("does not fall back to auto-detection when an explicit CODEX_OPENCODE_POOL_PATH override is missing", async () => {
			...
			expect(() => detectOpencodeAccountPoolPath()).toThrow(
				"CODEX_OPENCODE_POOL_PATH points to a file that does not exist: missing-pool.json",
			);
		...
		it("rejects Windows-style aliases of the active storage file as the opencode import source", async () => {
			const activeStoragePath = getStoragePath();
			await fs.writeFile(
				activeStoragePath,
				JSON.stringify({
			...
			const windowsAlias = activeStoragePath
				.replace(/\//g, "\\")
				.replace(/^([a-z]):/, (_, drive: string) => drive.toUpperCase() + ":");
			process.env.CODEX_OPENCODE_POOL_PATH = windowsAlias;
